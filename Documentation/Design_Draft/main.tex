...
Physical therapy for hand rehabilitation is a time- and cost-intensive process with limited at-home alternative options.
...
\paragraph{External Factors} Make sure the power connection is stable and not underloaded.
...
\item The device should be plugged into an AC stable power supply of at least 120V. A common wall socket is preferred.
\item Load any basic exercise and wait longer than 10 seconds.
\item Turn off the device by pressing the power button on the box connected to the wall. Make sure to press the power button before the exercise has finished.
...
\paragraph{Equipment} Hand device with external bank, Power Source, Separate device connected to internet and connected to device.
...
\paragraph{Goal} The device should weigh under 2 lbs or 1kg.
