...
CREATE TABLE users (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), role TEXT CHECK (role IN ('doctor', 'patient', 'admin')) NOT NULL, data JSONB);
...
CREATE TABLE admins (id UUID PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE);
